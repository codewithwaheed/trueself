...
async function getStats(apiToken: string) {
  try {
...
export default async function DashboardPage() {
  ...
  const showOnboarding =
    user.role === "ADMIN" && !user.onboardingComplete;
        ...
        <div className="card border border-[var(--border-subtle)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <a
              ...
              className="flex items-center gap-4 p-4 rounded-xl bg-navy-800/40 border border-[var(--border-subtle)] hover:border-trust/30 hover:bg-navy-800/60 transition-all group"
            >
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <span
          className="text-[10px] uppercase tracking-wider text-navy-500 font-semibold"
